...
class imageProcessor:
    ...
    def process_all_images(self):
        ...
        for i, org_img_path in enumerate(self.all_org_img_paths):
            ...
            img_np_format_clean = img_np_format
            img_np_format_clean[:, math.floor(org_img_width / 2):] = 0


            if imageProcessor._DEBUGGING:
                self.utils_helper.display_multi_image_collage(((img_np_format_clean, f"Image cleaned"),),
                                                              [1, 1])

            img_img_format_shifted = Image.fromarray(img_np_format_clean)
