...
class predictionProcessor:
    ...
    def __init__(self, original_predictions_path,
        ...
        self.original_predictions_path = original_predictions_path
        self.filter_threshold_array = filter_threshold_array
        self.experiment_name = experiment_name
        self.new_predictions_file_path = new_predictions_file_path
        self.middle_boundary = middle_boundary
        self.utils_helper = utils_helper
        self.logger = logger

    def read_predictions(self):
        with open(self.original_predictions_path) as json_file:
            self.org_predictions_data = json.load(json_file)

        self.new_predictions_data = copy.deepcopy(self.org_predictions_data)
        self.coco = COCO(self.original_predictions_path)
        self.logger.log(f"Loaded JSON prediction data from: {self.original_predictions_path}")

    def filter_predictions_w_wrong_area_ratio(self):
        #Filters the predictions which have a high-res-area/total-area ratio not compatible with the
        #current bin
        ...
        self.logger.log(f"Working on predictions bin {self.filter_threshold_array}")

        _anns_len = len(self.new_predictions_data["predictions"])
        self.ann_indices_to_keep = []
        for i, prediction in tqdm(enumerate(self.new_predictions_data["predictions"]),
                                  total = _anns_len,
                                  desc ="Progress for filtering predictions"):
            #---DEBUGGING---
            if predictionProcessor._DEBUGGING:
                pass
            #---------------
            self._bin_check_this_prediction(prediction, i)
            if int(i+1)%100==0:
                self.logger.log(f"Processed {i+1}/{_anns_len} predictions from {self.filter_threshold_array} bin")

        self.new_predictions_data["predictions"] = [self.new_predictions_data["predictions"][index] for index in
                                                    self.ann_indices_to_keep]


    def write_new_predictions_to_disk(self):
        self.utils_helper.write_data_to_json(self.new_predictions_file_path, self.new_predictions_data)
        self.logger.log(f"Successfully saved predictions for bin {self.filter_threshold_array} to disk. "
    ...
    def _bin_check_this_prediction(self, prediction, index):
        '''
        ...
        if predictionProcessor._VERBOSE:
            self.logger.log(f"Working on prediction with ID {index}: "
                            f" | Area {prediction['area']}"
                            f" | Image ID {prediction['image_id']}"
                            f" | prediction ID {prediction['id']}"
                            f" | Label {self.coco.loadCats(prediction['category_id'])}|")
        ...
        if predictionProcessor._DEBUGGING:
            self.utils_helper.display_multi_image_collage(((current_image_binary_mask_img, f"Image ID {prediction['image_id']}"), ),
                                                          [1, 1])
            pass
        ...
        if (high_res_area_fract >= self.filter_threshold_array[0]) and (high_res_area_fract <= self.filter_threshold_array[1]):
            ...
            if predictionProcessor._VERBOSE:
                self.logger.log(f"prediction {prediction['id']} on image {prediction['image_id']} was kept")
        else:
            if predictionProcessor._VERBOSE:
                self.logger.log(f"prediction {prediction['id']} on image {prediction['image_id']} was deleted")
