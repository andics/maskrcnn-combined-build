...
class Utilities_helper(object):
    ...
    def return_prediction_ids_present(self, json_target, to_print=False):

        with open(json_target) as json_file:
            json_data = json.load(json_file)

        prediction_ids_left = [x["id"] for x in json_data["predictions"]]

        if to_print:
            print("prediction IDs left: ", prediction_ids_left)

        return prediction_ids_left


    def extract_annotation_location_from_config(self, config):
        new_annotations_dir_path = os.path.join(config["FRAME"]["new_annotation_dir_path"],
                                                      config["LOGGING"]["logs_subdir"])
        return os.path.join(new_annotations_dir_path,
                                         self.extract_filename_and_ext(config["FRAME"]["org_annotation_file_path"])[0] + "." + \
                                         self.extract_filename_and_ext(
                                             config["FRAME"]["org_annotation_file_path"])[1])


    def extract_predictions_location_from_config(self, config):
        new_predictions_dir_path = os.path.join(config["FRAME"]["new_predictions_dir_path"],
                                                      config["LOGGING"]["logs_subdir"])
        return os.path.join(new_predictions_dir_path,
                                         self.extract_filename_and_ext(config["FRAME"]["org_predictions_file_path"])[0] + "." + \
                                         self.extract_filename_and_ext(
                                             config["FRAME"]["org_predictions_file_path"])[1])
    ...
    def find_element_with_prediction_id(self, json_object, prediction_id):
        prediction_id = str(prediction_id)
        #Delete the first character from the prediction ID as it correspons to the channel
        #print("Length of original prediction_id: ", len(prediction_id), "\n")
        #Remove the first element from the prediction ID to make them comparable => 1000234 becomes 000234
        prediction_id = str(prediction_id[1:])
        #print("Length of new prediction_id: ", len(prediction_id), "\n")

        for element in json_object:
            if prediction_id in str(element['id']):
