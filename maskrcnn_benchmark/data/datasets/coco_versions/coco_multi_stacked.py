...
class COCODataset(torchvision.datasets.coco.CocoDetection):
    def __init__(
        ...
        #Debugging
        self.img_total_c = np.array([0, 0, 0])
        self.total_imgs_passed = 0
    ...
    def __getitem__(self, idx):
        ...
        to_bgr = False
        #IMPORTANT: 0-3 CH1 and the rest are what logically follows
        ...
        self.total_imgs_passed = self.total_imgs_passed + 1
        self.img_total_c = self.img_total_c + np.mean(multi_ch_img[:, :, 6:9], axis=(0, 1))
        img_avg_c = self.img_total_c / self.total_imgs_passed
        print("Current tensor average color: ", img_avg_c.__str__())
        #if not to_bgr:
         #   multi_ch_img = np.flip(multi_ch_img, axis=2)
