...
class COCO_predictor(object):
    ...
    def compute_prediction(self, image_model_acceptable_format, image_tensor_like):
        ...
        image_model_acceptable_format = self.transforms(image_model_acceptable_format).to(self.cpu_device).unsqueeze(0)
        ...
        try:
            max_label = max(prediction.extra_fields["labels"].cpu().detach().numpy())
            print("Max label: ", max_label)

            if max_label > 81:
                print("We are here")
        except:
            print("Image had no predictions")
    ...
    def generate_class_names(self, image, predictions):
        ...
        for box, score, label in zip(boxes, scores, labels):
            ...
            str_for_img = str(prediction_numerator) + " - " + prediction_text

            cv2.putText(
                image, str_for_img, (int(x.item()), int(y.item())), cv2.FONT_HERSHEY_SIMPLEX, text_scale, (255, 255, 255), 1
            )
